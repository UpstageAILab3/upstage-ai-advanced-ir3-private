...
# OpenAI 및 Cohere 클라이언트 초기화


co = cohere.ClientV2(api_key="2c5icRN96L3jufQItrgcy4dsC8yQPCXR3R3Fvu6m")

# SetntenceTransformer를 이용하여 임베딩 생성
def get_embedding(sentences):
    client = OpenAI(
        api_key="up_jHe2kEAawCzGWNbtHGY0EzF5uH6od",
...
mappings = {
    "properties": {
        ...
        "docid": {"type": "keyword"}  # docid 필드 추가
    }
}
...
os.environ["OPENAI_API_KEY"] = "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"
client = OpenAI()
...
def answer_question(messages):
    ...
    if result.choices[0].message.tool_calls:
        ...
        search_result = dense_retrieve(standalone_query+'?', 10)  # top_k 10개
        ...
        for i, rst in enumerate(search_result['hits']['hits']):
            doc_content = rst["_source"]["content"]
            docid = rst["_source"].get("docid", f"doc_{i}")  # docid가 없을 경우 기본값 설정
            score = rst["_score"]
            retrieved_docs.append({"content": doc_content, "docid": docid, "score": score})

        # Cohere rerank를 사용하여 문서 재정렬
        ...
        # LLM에 전달할 컨텍스트 생성
        retrieved_context = [doc["content"] for doc in top_docs]
...
eval_rag("../data/eval.jsonl", "rerank_output.csv")
